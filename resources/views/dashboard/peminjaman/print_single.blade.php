...
<head>
    ...
    <style>
        ...
        /* Styling daftar dalam kolom agar tetap rata tengah */
        .list {
            list-style: none;
            padding: 0;
            margin: 0;
        ...
        /* Styling untuk item daftar agar ada spasi antar baris */
        .list li {
            margin: 2px 0;
        }
                    ...
                    <tr>
                        ...
                        @if ($peminjaman->kode_barang)
                            <td>
                                <ul class="list">
                                    @php
                                        $barangList = json_decode($peminjaman->kode_barang);
                                        $isMultipleBarang = count($barangList) > 1;
                                    @endphp
                                    @foreach ($barangList as $barangId)
                                        <li>{{ $isMultipleBarang ? '- ' : '' }}{{ \App\Models\DataBarang::find($barangId)->nama_barang ?? 'Data Barang tidak ditemukan' }}
                                        </li>
                                    @endforeach
                                </ul>
                            </td>
                        ...
                        @if ($peminjaman->kode_ruangan)
                            <td>
                                <ul class="list">
                                    @php
                                        $ruanganList = json_decode($peminjaman->kode_ruangan);
                                        $isMultipleRuangan = count($ruanganList) > 1;
                                    @endphp
                                    @foreach ($ruanganList as $ruanganId)
                                        <li>{{ $isMultipleRuangan ? '- ' : '' }}{{ \App\Models\DataRuangan::find($ruanganId)->nama_ruangan ?? 'Data Ruangan tidak ditemukan' }}
                                        </li>
                                    @endforeach
                                </ul>
                            </td>
